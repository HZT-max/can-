...
#define UART_IRQ_RX_BUFFER_SIZE     256U
#define UART_RX_DRAIN_LIMIT         256U
...
#define SYSCFG_CFGR1                (*(volatile uint32_t *)0x40010000U)
#define SYSCFG_CFGR1_USART3_DMA_RMP (1U << 26)

#define NVIC_ISER                   (*(volatile uint32_t *)0xE000E100U)
#define USART1_IRQ_NUMBER           27U
...
#define DMA1_CHANNEL3              ((DMAChannelRegisters *)0x40020030U)
#define DMA1_CHANNEL5              ((DMAChannelRegisters *)0x40020058U)
#define DMA1_CHANNEL6              ((DMAChannelRegisters *)0x4002006CU)
...
typedef struct {
    ...
    uint16_t tx_head;
    uint16_t tx_tail;
    DMAChannelRegisters *rx_dma;
    volatile uint8_t *rx_dma_buffer;
    uint16_t rx_dma_read_index;
    volatile uint8_t *irq_rx_buffer;
    volatile uint16_t irq_rx_head;
    volatile uint16_t irq_rx_tail;
...
const vector_handler_t vector_table[16U + 30U] = {
    [0] = (vector_handler_t)&_estack,
    [1] = Reset_Handler,
    [2] = Default_Handler,
    [3] = Default_Handler,
    [4] = Default_Handler,
    ...
    [14] = Default_Handler,
    [15] = SysTick_Handler,
    [16U + USART1_IRQ_NUMBER] = USART1_IRQHandler,
};
...
static volatile uint8_t g_uart1_irq_rx_buffer[UART_IRQ_RX_BUFFER_SIZE];
static volatile uint8_t g_uart2_dma_rx_buffer[UART_DMA_RX_BUFFER_SIZE];
static volatile uint8_t g_uart3_dma_rx_buffer[UART_DMA_RX_BUFFER_SIZE];
static volatile uint8_t g_uart4_dma_rx_buffer[UART_DMA_RX_BUFFER_SIZE];
static UARTPort g_uart_ports[UART_PORT_COUNT] = {
    {
        .regs = USART1_REGS,
        .baudrate = UART_DEFAULT_BAUDRATE,
        .serial_id = 1U,
        .irq_rx_buffer = g_uart1_irq_rx_buffer,
    },
    {
        .regs = USART2_REGS,
        .baudrate = UART_DEFAULT_BAUDRATE,
        ...
        .rx_dma_buffer = g_uart2_dma_rx_buffer,
    },
    {
        .regs = USART3_REGS,
        .baudrate = UART_DEFAULT_BAUDRATE,
        ...
        .rx_dma_buffer = g_uart3_dma_rx_buffer,
    },
    {
        .regs = UART4_REGS,
        .baudrate = UART_DEFAULT_BAUDRATE,
        ...
        .rx_dma_buffer = g_uart4_dma_rx_buffer,
    },
};

_Static_assert((UART_DMA_RX_BUFFER_SIZE & (UART_DMA_RX_BUFFER_SIZE - 1U)) == 0U,
               "DMA RX buffer size must be a power of two");
_Static_assert((UART_IRQ_RX_BUFFER_SIZE & (UART_IRQ_RX_BUFFER_SIZE - 1U)) == 0U,
               "IRQ RX buffer size must be a power of two");
...
/*
 * uart_apply_baudrate()在上电和飞控改变隧道波特率时调用。
 * UART2/3/4在这里重新启动循环DMA；UART1重新启用RXNE中断。
 */
static void uart_start_rx(UARTPort *port)
{
    if ((port->rx_dma != NULL) && (port->rx_dma_buffer != NULL)) {
        port->rx_dma->CCR = 0U;
        port->rx_dma->CPAR = (uint32_t)(uintptr_t)&port->regs->RDR;
        port->rx_dma->CMAR = (uint32_t)(uintptr_t)port->rx_dma_buffer;
        port->rx_dma->CNDTR = UART_DMA_RX_BUFFER_SIZE;
        port->rx_dma_read_index = 0U;
        port->regs->CR3 |= USART_CR3_DMAR;
        port->rx_dma->CCR = DMA_CCR_CIRC | DMA_CCR_MINC | DMA_CCR_PL_HIGH | DMA_CCR_EN;
        return;
    }

    if (port->irq_rx_buffer != NULL) {
        port->irq_rx_head = 0U;
        port->irq_rx_tail = 0U;
        port->regs->CR1 |= USART_CR1_RXNEIE;
    }
}
...
/*
 * 四路UART均使用静态缓冲，不创建线程或动态内存。
 * UART2_RX=DMA1通道5，UART3_RX重映射到通道3，UART4_RX=通道6。
 */
static void uart_init_all(void)
{
    ...
    NVIC_ISER = (1UL << USART1_IRQ_NUMBER);
}

/* USART1每收到一个字节就只写入静态环形缓冲，协议封装仍由主循环完成。 */
void USART1_IRQHandler(void)
{
    UARTPort *port = &g_uart_ports[0];
    const uint32_t status = port->regs->ISR;

    if ((status & USART_ISR_RXNE) != 0U) {
        const uint8_t byte = (uint8_t)port->regs->RDR;
        const uint16_t next =
            (uint16_t)((port->irq_rx_head + 1U) & (UART_IRQ_RX_BUFFER_SIZE - 1U));
        if (next != port->irq_rx_tail) {
            port->irq_rx_buffer[port->irq_rx_head] = byte;
            port->irq_rx_head = next;
        } else {
            port->dropped_rx_bytes++;
        }
    }

    const uint32_t error_flags = status & USART_ERROR_MASK;
    if (error_flags != 0U) {
...
/* 从循环DMA或UART1中断环形缓冲中取出一个字节，取不到时返回false。 */
static bool uart_read_rx_byte(UARTPort *port, uint8_t *byte)
{
    if ((port->rx_dma != NULL) && (port->rx_dma_buffer != NULL)) {
        const uint32_t remaining = port->rx_dma->CNDTR;
        if (remaining > UART_DMA_RX_BUFFER_SIZE) {
            return false;
        }

        const uint16_t write_index =
            (uint16_t)((UART_DMA_RX_BUFFER_SIZE - remaining) & (UART_DMA_RX_BUFFER_SIZE - 1U));
        if (port->rx_dma_read_index == write_index) {
            return false;
        }

        *byte = port->rx_dma_buffer[port->rx_dma_read_index];
        port->rx_dma_read_index =
            (uint16_t)((port->rx_dma_read_index + 1U) & (UART_DMA_RX_BUFFER_SIZE - 1U));
        return true;
    }

    if ((port->irq_rx_buffer != NULL) && (port->irq_rx_tail != port->irq_rx_head)) {
        *byte = port->irq_rx_buffer[port->irq_rx_tail];
        port->irq_rx_tail =
            (uint16_t)((port->irq_rx_tail + 1U) & (UART_IRQ_RX_BUFFER_SIZE - 1U));
        return true;
    }
...
/* 读取UART接收缓冲；满120字节或空闲2 ms时封装成一条DroneCAN隧道消息。 */
static void uart_process_rx(UARTPort *port)
{
    ...
    uint16_t reads = 0U;
    uint8_t byte;
    while ((reads < UART_RX_DRAIN_LIMIT) && uart_read_rx_byte(port, &byte)) {
        reads++;
...
static void app_main(void)
{
    ...
    while (true) {
        ...
        for (uint8_t i = 0U; i < UART_PORT_COUNT; i++) {
            ...
            uart_process_rx(&g_uart_ports[i]);
        }
