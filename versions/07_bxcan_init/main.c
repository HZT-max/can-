...
static volatile int16_t can_init_result;
...
/* canard_stm32在等待bxCAN进出初始化模式时需要1 ms延时。 */
int usleep(unsigned int usec)
{
    const uint32_t wait_ms = (usec + 999U) / 1000U;
    const uint32_t start_ms = systick_millis;
    while ((uint32_t)(systick_millis - start_ms) < wait_ms) {
    }
    return 0;
}
...
/* 本阶段只启动PB8/PB9上的bxCAN，固定1 Mbit/s，不发送DroneCAN消息。 */
static uint32_t bxcan_init_1mbit(void)
{
    gpio_set_alternate(&GPIOB_MODER, &GPIOB_OTYPER, &GPIOB_OSPEEDR, &GPIOB_PUPDR,
    ...
    can_init_result = canardSTM32ComputeCANTimings(48000000U, 1000000U, &timings);
    if (can_init_result < 0) {
        return 0U;
    }

    can_init_result = canardSTM32Init(&timings, CanardSTM32IfaceModeNormal);
    return can_init_result == 0;
}
