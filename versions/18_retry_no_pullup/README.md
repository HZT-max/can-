...
本版本以第17版为基础，用于验证CAN收发器TX/RX飞线修正后的硬件。
继续取消PB8 CAN_RX内部上拉，但恢复CAN初始化失败后的循环重试。
...
CAN初始化失败时每100 ms重试一次，成功前RUN灯保持熄灭；初始化成功后RUN每1秒翻转。PB8不启用内部上拉，直接读取飞线修正后U6 RXD输出的真实总线状态。
