...
本版本保留第12版全部业务功能、CAN_RX弱上拉和CAN初始化持续重试。
唯一诊断变化是RUN灯不再表示“业务主循环正在运行”，而是读取bxCAN硬件发送状态。
...
- 每1秒翻转一次：至少一帧CAN已得到总线上其他节点的ACK，物理发送成功。
- 每0.1秒快速翻转：发送错误计数增长，常见原因是无ACK、波特率不一致、接线或收发器问题。
- 始终不亮：CAN尚未成功初始化，或者发送请求尚未产生硬件成功/错误结果。
...
本版用于定位“RUN正常但Mission Planner没有节点125”，不是最终产品灯态。
