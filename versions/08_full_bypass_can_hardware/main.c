...
/* 诊断版故意跳过CAN硬件接收，避免访问未初始化的bxCAN。 */
static void can_process_rx(void)
{
}

/* 保留DroneCAN编码和队列路径，但将待发CAN帧丢弃，避免内存池被占满。 */
static void can_process_tx(void)
{
    while (canardPeekTxQueue(&g_canard) != NULL) {
        canardPopTxQueue(&g_canard);
    }
}

/* NodeStatus是节点在线心跳；PB1独立翻转，CAN拥塞时仍可确认程序在运行。 */
...
static void app_main(void)
{
    ...
    /* 本版不调用canardSTM32Init()，用来隔离已确认失败的CAN硬件层。 */
