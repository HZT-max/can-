...
#define CAN_ESR_TEC_MASK            (0xFFU << 16U)
...
/* NodeStatus是节点在线心跳；本诊断版由CAN硬件状态单独控制RUN灯。 */
...
/*
 * 主循环在每次CAN发送后调用：TXOK慢闪表示帧得到ACK；错误计数增长时快闪。
 * 尚未发送成功且没有硬件错误时保持熄灭，避免把“进入主循环”误认为“CAN已通信”。
 */
static void update_can_diagnostic_led(void)
{
    static uint32_t last_toggle_ms;
    const uint32_t now_ms = millis();
    const uint32_t esr = CAN_ESR;

    if ((CAN_TSR & CAN_TSR_TXOK_MASK) != 0U) {
        g_can_tx_ok_seen = true;
    }

    if (((esr & CAN_ESR_BOFF) != 0U) || ((esr & CAN_ESR_TEC_MASK) != 0U)) {
        if ((uint32_t)(now_ms - last_toggle_ms) >= 100U) {
            last_toggle_ms = now_ms;
            GPIOB->ODR ^= (1U << 1); // 快闪：发送错误、无ACK或总线关闭
        }
        return;
    }

    if (g_can_tx_ok_seen) {
        if ((uint32_t)(now_ms - last_toggle_ms) >= 1000U) {
            last_toggle_ms = now_ms;
            GPIOB->ODR ^= (1U << 1); // 慢闪：至少一帧已被其他CAN节点确认
        }
        return;
    }
